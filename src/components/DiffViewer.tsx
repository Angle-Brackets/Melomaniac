      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          ...
          <div className="rounded bg-base-200 p-3 text-sm font-mono">
            {oursPoints
              ? `A: ${fmtMs(oursPoints.ab_a)} / B: ${fmtMs(oursPoints.ab_b)}`
              : '—'
            }
          </div>
        </div>
        <div>
          ...
          <div className="rounded bg-base-200 p-3 text-sm font-mono">
            {theirsPoints
              ? `A: ${fmtMs(theirsPoints.ab_a)} / B: ${fmtMs(theirsPoints.ab_b)}`
              : '—'
            }
          </div>
        </div>
...
function DiffViewerInner({ platform }: { platform: 'desktop' | 'mobile' }) {
    ...
    <div className="flex items-center justify-between mb-4">
      <div>
        ...
        <div className="font-semibold text-sm truncate max-w-xs">
          "{playlistName}"
          {!done && total > 0 && (
            <span className="ml-2 opacity-50 font-normal">{currentIdx + 1} of {total}</span>
          )}
        </div>
      </div>
      <button
  ...
  if (platform === 'mobile') {
    return (
      <div className="fixed inset-0 z-50 flex flex-col justify-end">
        ...
        />
        <div
          className="relative bg-base-100 rounded-t-2xl p-5 pb-safe"
  ...
  // desktop: fixed full-screen overlay styled as a DaisyUI modal
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      <div
        ...
        onClick={closeDiffViewer}
      />
      <div className="relative bg-base-100 rounded-xl shadow-2xl w-full max-w-lg mx-4 p-6 max-h-[80vh] overflow-y-auto">
        {header}
        {body}
      </div>
    </div>
...
export function DiffViewer({ platform }: { platform: 'desktop' | 'mobile' }) {
  ...
  if (!open) return null
  return <DiffViewerInner platform={platform} />
}
