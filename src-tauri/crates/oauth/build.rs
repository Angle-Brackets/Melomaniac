...
fn compile_swift() {
    ...
    // fall back to sensible minimums that cover AuthenticationServices APIs.
    ...
    println!("cargo:rerun-if-changed=ios/Sources/MelomaniacOAuth/MelomaniacOAuth.swift");
    ...
    SwiftLinker::new(&macos_min)
        .with_ios(&ios_min)
        .with_package("MelomaniacOAuth", &ios_dir)
        .link();
